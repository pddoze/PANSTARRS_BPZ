...
find ~/Projects/Work/testFiles -name '*.txt' |  while read FILENAME; do #FILENAME still has path
#	echo "$FILENAME"
	NEWFILENAME=$(basename $FILENAME)  #takes path out, only file name left
#	echo "$NEWFILENAME"
	sed -e "s/5/banana/g" <$FILENAME >~/Projects/Work/testFiles2/${NEWFILENAME%.txt}bat.txt 
done 
